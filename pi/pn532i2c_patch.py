...
class Pn532I2c(Pn532Interface):
    ...
    def _readAckFrame(self) -> int:
        ...
        if payload != PN532_ACK:
            # PN532 sent the response frame directly instead of a separate ACK
            # (common on faster hosts like Pi 5). Send NACK so PN532 re-queues
            # the frame for _getResponseLength to read.
            DMSG("No ACK — got response early, sending NACK to re-queue\n")
            try:
                self._wire.transaction(writing(PN532_I2C_ADDRESS, PN532_NACK))
            except IOError as e:
